...
The checked-in `.github/workflows/playwright.yml` detects whether `ANTHROPIC_AUTH_TOKEN` is configured for the repository. When the token is unavailable, such as on forked pull requests, the workflow creates a successful job and skips AI-backed Playwright execution. If you want a pure generated-test workflow with no AI dependency, use the example above and point Playwright at tests that do not call the generation pipeline.
