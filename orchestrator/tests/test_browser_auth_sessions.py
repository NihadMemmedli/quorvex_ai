...
def _create_active_browser_auth_session(
    ...
    *,
    ...
    state = state or {
        "cookies": [{"name": "sid", "value": f"cookie-{uuid4().hex}", "domain": "example.com", "path": "/"}],
        "origins": [],
    }
    with Session(engine) as session:
...
def test_resolve_browser_auth_for_run_writes_decrypted_state(tmp_path):
    ...
    with Session(engine) as session:
        ...
        resolved = resolve_browser_auth_for_run(session, project_id, run_dir=tmp_path, use_default=True)
...
def test_create_run_resolves_explicit_browser_auth_session_into_temporal_payload(tmp_path, monkeypatch):
    ...
    auth_session_id = _create_active_browser_auth_session(
        project_id,
        state={"cookies": [{"name": "sid", "value": "single-run", "domain": "example.com", "path": "/"}], "origins": []},
    )
    specs_dir = tmp_path / "specs"
    runs_dir = tmp_path / "runs"
    specs_dir.mkdir()
    runs_dir.mkdir()
    (specs_dir / "checkout.md").write_text("# Checkout\n\n## Steps\n1. Open https://example.com")
...
def test_create_run_without_browser_auth_leaves_temporal_payload_unchanged(tmp_path, monkeypatch):
    ...
    specs_dir.mkdir()
    runs_dir.mkdir()
    (specs_dir / "public.md").write_text("# Public\n\n## Steps\n1. Open https://example.com")
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        response = client.post("/runs", json={"spec_name": "public.md", "project_id": project_id})
...
def test_bulk_run_writes_one_storage_state_file_per_run_for_project_default(tmp_path, monkeypatch):
    ...
    _create_active_browser_auth_session(
        project_id,
        is_default=True,
        state={"cookies": [{"name": "sid", "value": "bulk-run", "domain": "example.com", "path": "/"}], "origins": []},
    )
    specs_dir = tmp_path / "specs"
    runs_dir = tmp_path / "runs"
    specs_dir.mkdir()
    ...
    storage_paths = [Path(payload["storage_state_path"]) for payload in started_payloads]
...
def test_run_rejects_cross_project_browser_auth_session_before_temporal_start(tmp_path, monkeypatch):
    ...
    specs_dir.mkdir()
    runs_dir.mkdir()
    (specs_dir / "private.md").write_text("# Private\n\n## Steps\n1. Open https://example.com")
...
def test_browser_auth_session_api_lifecycle():
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        ...
        validate_response = client.post(f"/projects/{project_id}/browser-auth-sessions/{session_id}/validate")
        assert validate_response.status_code == 200, validate_response.text
        assert validate_response.json()["status"] == "active"

        default_response = client.patch(f"/projects/{project_id}/browser-auth-sessions/{session_id}/default")
        assert default_response.status_code == 200, default_response.text
        assert default_response.json()["is_default"] is True

        delete_response = client.delete(f"/projects/{project_id}/browser-auth-sessions/{session_id}")
...
def test_browser_auth_session_api_round_trips_advanced_capture_settings():
    ...
    with TestClient(app, raise_server_exceptions=False) as client:
        create_response = client.post(f"/projects/{project_id}/browser-auth-sessions", json=payload)
        list_response = client.get(f"/projects/{project_id}/browser-auth-sessions")
...
def test_browser_auth_session_create_mcp_failure_returns_400_and_persists_invalid(monkeypatch):
    from orchestrator.api.main import app
    from orchestrator.services import browser_auth_sessions as service

    project_id = _create_uuid_project()
    with Session(engine) as session:
        assert set_project_credential(project_id, "LOGIN_USERNAME", "user@example.com", session)
        assert set_project_credential(project_id, "LOGIN_PASSWORD", "secret-password", session)

    def fail_mcp_capture(**_kwargs):
        raise BrowserAuthSessionError("security challenge detected")

    monkeypatch.setattr(service, "capture_storage_state_via_mcp_agent", fail_mcp_capture)
...
def test_browser_auth_session_create_reactivates_revoked_same_name_row(monkeypatch):
    ...
    project_id = _create_uuid_project()
    old_state = {"cookies": [{"name": "old", "value": "stale", "domain": "example.com", "path": "/"}], "origins": []}
    ...
    def fake_mcp_capture(**kwargs):
        ...
        return {"cookies": [{"name": "sid", "value": "fresh", "domain": "pre.wetravel.to", "path": "/"}], "origins": []}

    monkeypatch.setattr(service, "capture_storage_state_via_mcp_agent", fake_mcp_capture)
    ...
    with Session(engine) as session:
        ...
        def fail_first_commit():
            ...
            if calls["count"] == 1:
                ...
                raise IntegrityError("INSERT INTO browser_auth_sessions", {}, Exception("duplicate"))
...
def test_refresh_browser_auth_session_uses_mcp_capture_and_passes_advanced_selectors(monkeypatch):
    from orchestrator.services import browser_auth_sessions as service
...
def test_capture_storage_state_via_mcp_agent_reads_and_validates_storage_state(tmp_path, monkeypatch):
    from orchestrator.services import browser_auth_sessions as service

    monkeypatch.setattr(service, "AUTH_SESSIONS_DIR", tmp_path)
    captured: dict[str, object] = {}

    def fake_run_async_capture(prompt, *, run_dir, timeout_seconds, session_id):
...
def test_capture_storage_state_via_mcp_agent_missing_file_normalizes_failure(tmp_path, monkeypatch):
    from orchestrator.services import browser_auth_sessions as service
...
def test_mcp_capture_normalizes_old_direct_helper_error_text(monkeypatch):
    ...
    normalized = service._normalize_capture_error("locator.fill timeout at [eval]:35:25")
...
def test_run_capture_agent_uses_settings_backed_zai_key_for_preflight(tmp_path, monkeypatch):
    from orchestrator.services import browser_auth_sessions as service
    ...
    result = asyncio.run(
        service._run_capture_agent("capture login", run_dir=tmp_path, timeout_seconds=5, session_id="session-settings-key")
    )
...
def test_run_capture_agent_missing_zai_key_has_actionable_message(tmp_path, monkeypatch):
    from orchestrator.services import browser_auth_sessions as service
    ...
    try:
        asyncio.run(
            service._run_capture_agent("capture login", run_dir=tmp_path, timeout_seconds=5, session_id="session-no-key")
        )
    except BrowserAuthSessionError as exc:
        message = str(exc)
    else:
    ...
    assert "LLM provider API key is not configured for Z.ai/GLM" in message
    assert "Settings or deployment environment secrets" in message
    assert "restart the backend and worker services if using environment secrets" in message
